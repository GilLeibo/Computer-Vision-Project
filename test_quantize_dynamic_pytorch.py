...
def main():
    ...
    if not distributed:
        ...
        tegrastats_MEM_data = pd.read_csv('quantization_recordings/tmp2.txt', sep=' ', usecols=[3], names=["Pytorch_Post_Training_Dynamic_Quantization(int8)"])
        ...
        tegrastats_Power_data = pd.read_csv('quantization_recordings/tmp2.txt', sep=' ', usecols=[32], names=["Pytorch_Post_Training_Dynamic_Quantization(int8)"])
    ...
    cmd = 'printf "' + network_name + ' ' + power_mode_str + ' ' + "Pytorch_Post_Training_Dynamic_Quantization(int8)" + ' ' + elapsed_inference + ' ' + MEM_median + ' ' + MEM_avg + ' ' + GPU_power_median + ' ' + GPU_power_avg + ' ' + model_size + ' ' + mIoU_result + '\n"' + ' >> ' + 'quantization_recordings/quantization_data.txt'
    subprocess.run(cmd, shell=True)
