...
def single_gpu_test_modified(model,
                    data_loader,
                    show=False,
                    out_dir=None,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            ...
            cmd = 'sudo tegrastats --interval 10 --logfile tegrastats_recordings/' + tegra_filename + '.txt'
    ...
    # write tegrastats TPP results to the current power_mode TPP csv file
    power_mode_TPP_data = pd.read_csv('tegrastats_recordings/' + power_mode_str + '_TPP.csv', header=0)
    tegrastats_TPP_data = pd.DataFrame(pictures_TIMES, columns=[network_name])
    TPP_result = pd.concat([power_mode_TPP_data, tegrastats_TPP_data], axis=1)
    TPP_result.to_csv('tegrastats_recordings/' + power_mode_str + '_TPP.csv', index=False)
    ...
    tegrastats_MEM_data = pd.DataFrame(pictures_MEM_AVG, columns=[network_name])
    ...
    tegrastats_Power_data = pd.DataFrame(pictures_POWER_AVG, columns=[network_name])
